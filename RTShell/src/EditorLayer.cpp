...
#include <Remc/Scene/SceneSerializer.h>
#include <Remc/Utils/PlatformUtils.h>

#include <ImGuizmo.h>

#include <Remc/Math/Math.h>
...
namespace Remc {
	...
	void EditorLayer::OnUpdate(Timestep ts)
	{
		...
		if (mouseX >= 0 && mouseY >= 0 && mouseX < (int)viewportSize.x && mouseY < (int)viewportSize.y)
		{
			...
			REMC_CORE_WARN("Pixel data = {0}", pixelData);
		}
	...
	void EditorLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<KeyPressedEvent>(REMC_BIND_EVENT_FN(EditorLayer::OnKeyPressed));
	}
	...
	{
		std::optional<std::string> filepath = FileDialogs::OpenFile("Remc Scene (*.remc)\0*.remc\0");
		if (filepath)
		{
			m_ActiveScene = CreateRef<Scene>();
			m_ActiveScene->OnViewportResize((uint32_t)m_ViewportSize.x, (uint32_t)m_ViewportSize.y);
	...
	{
		std::optional<std::string> filepath = FileDialogs::SaveFile("Remc Scene (*.remc)\0*.remc\0");
		if (filepath)
		{
			SceneSerializer serializer(m_ActiveScene);
